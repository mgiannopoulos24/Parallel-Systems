...
def run_test(exec_path: Path, implementation_name: str, csv_writer: csv.writer):
    """
    Runs the specified implementation executable with different thread counts,
    measures execution times, and records the results to the CSV file.
    ...
    for threads in THREAD_COUNTS:
        ...
        print(f"{'Implementation':<20} {'Threads':<10} {'Run':<10} {'Execution Time (s)':<20}")
        print(f"{'-'*20} {'-'*10} {'-'*10} {'-'*20}")
        ...
        for run in range(1, RUNS_PER_TEST + 1):
            ...
            try:
                ...
                subprocess.run(
                    [str(exec_path), str(threads)],
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL,
                    check=True
                )
            except subprocess.CalledProcessError as e:
                print(f"Error: Execution failed for {implementation_name} with {threads} threads on run {run}")
            ...
            print(f"{implementation_name:<20} {threads:<10} {run:<10} {elapsed_time:<20.5f}")

            # Save the result to the CSV (Average Time column left empty for individual runs)
            csv_writer.writerow([implementation_name, threads, run, f"{elapsed_time:.5f}", ""])

        # Calculate and print the average execution time
        avg_time = total_time / RUNS_PER_TEST
        print(f"Average time for {implementation_name} with {threads} threads: {avg_time:.5f} seconds")

        # Append average time to the CSV
        csv_writer.writerow([implementation_name, threads, "Average", "", f"{avg_time:.5f}"])
...
def main():
    ...
    try:
        with open(OUTPUT_CSV, mode='w', newline='') as csvfile:
            ...
            # Write the header
            csv_writer.writerow(["Implementation", "Threads", "Run", "Execution Time (s)", "Average Time (s)"])

            # Iterate over each valid implementation
            for implementation_name, exec_path in valid_implementations.items():
                run_test(exec_path, implementation_name, csv_writer)
