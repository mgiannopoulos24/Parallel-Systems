...
int main(int argc, char *argv[]) {
    ...
    for (int gen = 0; gen < num_generations; gen++) {
        ...
        MPI_Status top_status, bottom_status;

        // Non-blocking communication for borders
        if (rank > 0) {
            MPI_Isend(local_current, grid_size, MPI_INT, rank - 1, 0, MPI_COMM_WORLD, &top_request);
            MPI_Irecv(top_row, grid_size, MPI_INT, rank - 1, 0, MPI_COMM_WORLD, &top_request);
        }
        if (rank < size - 1) {
            MPI_Isend(&local_current[(rows_per_process - 1) * grid_size], grid_size, MPI_INT, rank + 1, 0, MPI_COMM_WORLD, &bottom_request);
            MPI_Irecv(bottom_row, grid_size, MPI_INT, rank + 1, 0, MPI_COMM_WORLD, &bottom_request);
        }

        // Compute next generation during communication
        ...
        if (rank > 0) {
            MPI_Wait(&top_request, &top_status);
        }
        if (rank < size - 1) {
            MPI_Wait(&bottom_request, &bottom_status);
        }
